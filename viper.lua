...
developers = {
    "@ l e n d a - l e n d a#2964"
}


menuName = "~h~~w~• V I P E R •"
version = "1.0" 
theme = "basic"
...
startMessage = "~g~∑ ~s~~h~Welcome → ~g~[~s~~h~"..GetPlayerName(PlayerId()).."~h~~g~]"	
--subMessage = "~n~~w~🔒 OPEN MENU 🔒"
--subMessage1 = "🔑 ~g~[ ~s~"..menuKeybind.." ~g~]~s~]"
motd = "~g~∑ ~s~~h~lenda#2964 → ~g~[~s~~h~Discord~h~~g~]"
motd2 = "~g~~h~N4~s~~h~ → ~g~[~s~~h~TPWaypoint~g~]"
motd3 = "~g~~h~N5~s~~h~ → ~g~[~s~~h~Noclip~g~]"
motd4 = "~g~~h~N6~s~~h~ → ~g~[~s~~h~FixVehicle~g~]"
...
Citizen.CreateThread(function()
    local p = 1
    while true do
        if theme == "skid" then -- static effect for basic theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
            if p == 5 then p = 1 else p = p + 1 end
            for i = 1, #menulist do
                if MIOddhwuie.IsMenuOpened(menulist[i]) then MIOddhwuie.SetTitleBackgroundSprite(menulist[i], 'commonmenu', 'interaction_bgd' .. p) end
            end
        else -- Base textures
            for i = 1, #menulist do MIOddhwuie.SetTitleBackgroundSprite(menulist[i], 'shopui_title_sm_hangar', 'shopui_title_sm_hangar') end
        end
        Wait(100)
    end
end)
...
Citizen.CreateThread(function()
    if mpMessage then ShowMPMessage(startMessage, 50) else ShowInfo(startMessage) end
--	if mpMessage then ShowMPMessage(subMessage1, 50) else ShowInfo(subMessage1) end
    ...
    MIOddhwuie.SetSubTitle('skid', 'Server IP: ' .. GetCurrentServerEndpoint())
